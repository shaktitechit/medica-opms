...
export function OrderStatusDimensionCell({ dimension, className = "" }: Props) {
  ...
  return (
    <td className={`px-4 py-3 align-top ${className}`}>
      <span
        className={`inline-flex max-w-[11rem] truncate rounded-full px-2 py-0.5 text-[10px] font-semibold ring-1 ${dimensionToneClass(dimension.tone)}`}
        title={title}
      >
        {dimension.label}
      </span>
      {dimension.detail ? (
        <p
          className="mt-0.5 max-w-[11rem] truncate text-[9px] text-slate-500 dark:text-slate-400"
