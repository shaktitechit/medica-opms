...
export function BulkUploadPartiesModal({
  ...
  const parseCSV = (text: string): ParsedParty[] => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Map to columns
      const name = raw.party_name || raw.name || "";
      const party_type_raw = raw.party_type || raw.type || "customer";
      const contact_person = raw.contact_person || raw.contact || "";
      const mobile = raw.mobile || raw.phone || "";
      const email = raw.email || "";
      const gst_no = raw.gst_no || raw.gst || raw.gstin || "";
      const drug_license_no = raw.drug_license_no || raw.dl_no || "";
      const district = raw.district || "";
      const state = raw.state || "";
      const payment_terms = raw.payment_terms || "";

      // Validate
      let status: "ready" | "invalid" = "ready";
      let reason = "";

      const party_type = party_type_raw.toLowerCase().trim() as any;
      ...
      } else if (party_type_raw && !validTypes.includes(party_type)) {
        status = "invalid";
      ...
      parsed.push({
        party_name: name,
        party_type: validTypes.includes(party_type) ? party_type : "customer",
        contact_person,
        mobile,
        email,
        gst_no,
    ...
    reader.onload = (e) => {
      ...
      try {
        if (selectedFile.name.endsWith(".json")) {
          ...
          const mapped = arr.map((item: any): ParsedParty => {
            const name = item.party_name || item.name || "";
            const party_type_raw = item.party_type || item.type || "customer";
            const contact_person = item.contact_person || item.contact || "";
            const mobile = item.mobile || item.phone || "";
            const email = item.email || "";
            const gst_no = item.gst_no || item.gst || item.gstin || "";
            const drug_license_no = item.drug_license_no || item.dl_no || "";
            const district = item.district || "";
            const state = item.state || "";
            const payment_terms = item.payment_terms || "";
  ...
  const downloadTemplate = useCallback(() => {
    const csvContent =
      "party_name,party_type,contact_person,mobile,email,gst_no,drug_license_no,district,state,payment_terms\n" +
      '"Apollo Pharmacy","customer","Dr. Rajesh Kumar","+91 9999988888","billing@apollomed.com","07AAAAA1111A1Z1","DL-12345/6789","Central Delhi","Delhi","Net 30"\n' +
      '"Cipla Ltd","supplier","Mr. Amit Shah","+91 8888877777","sales@cipla.com","27BBBBB2222B2Z2","DL-67890/1234","Mumbai","Maharashtra","Net 45"\n' +
      '"Med Plus","both","Mrs. Shalini Rao","+91 7777766666","contact@medplus.com","33CCCCC3333C3Z3","DL-54321/9876","Bangalore","Karnataka","Cash on Delivery"';
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-blue-100 bg-blue-50/50 p-3.5 dark:border-blue-950/40 dark:bg-blue-950/10">
            <div className="text-xs text-blue-800 dark:text-blue-300 max-w-xl space-y-1">
              ...
              <p>
                First row must contain header columns: <code className="font-mono bg-blue-100/80 px-1 py-0.5 rounded dark:bg-blue-950">party_name</code> (required), <code className="font-mono bg-blue-100/80 px-1 py-0.5 rounded dark:bg-blue-950">mobile</code> (required), <code className="font-mono bg-blue-100/80 px-1 py-0.5 rounded dark:bg-blue-950">party_type</code> (customer / supplier / both), contact_person, email, gst_no, etc.
              </p>
            </div>
            <button
              type="button"
                ...
                <table className="w-full text-left text-xs min-w-[1000px]">
                  <thead className="sticky top-0 bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400 border-b border-slate-200/90 dark:border-white/10">
                    <tr>
                      ...
                      <th className="px-3 py-2 font-medium">Drug License</th>
                  ...
                  <tbody className="divide-y divide-slate-200/80 dark:divide-white/10">
                    {parsedParties.map((p, idx) => (
                      ...
                      >
                        ...
                        <td className={`px-3 py-2 font-mono text-[11px] max-w-[120px] truncate ${p.status === "invalid" && !p.mobile ? "text-rose-600 bg-rose-50/30 dark:text-rose-400" : ""}`}>
                          {p.mobile || "— (Missing)"}
                        </td>
