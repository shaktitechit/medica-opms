...
import {
  type ProductManufacturerRecord,
...
export type ListProductManufacturersPageProps = {
  portalHome?: string;
};

export default function ListProductManufacturersPage({
  portalHome = "/admin",
}: ListProductManufacturersPageProps) {
  const router = useRouter();
  const { data, isLoading, isFetching, isError, refetch } = useListProductManufacturersQuery({
    limit: 1000,
  });
  const rows = useMemo(() => (Array.isArray(data?.data) ? (data.data as ProductManufacturerRecord[]) : []), [data]);

  const [createProductManufacturer] = useCreateProductManufacturerMutation();
  const [patchProductManufacturer] = usePatchProductManufacturerMutation();
  const [deleteProductManufacturer] = useDeleteProductManufacturerMutation();
  ...
  // Modals state
  const [editTarget, setEditTarget] = useState<ProductManufacturerRecord | null>(null);
  const [createOpen, setCreateOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<ProductManufacturerRecord | null>(null);
  ...
  const openEdit = (man: ProductManufacturerRecord) => {
    setEditTarget(man);
    setFormName(man.name);
    setFormDesc(man.description || "");
    setFormActive(man.is_active);
    setFormFeatured(!!man.is_featured);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editTarget) {
        await patchProductManufacturer({
          id: editTarget._id,
        ...
        await createProductManufacturer({
          name: formName.trim(),
      ...
      toast.error(err?.data?.message || "Failed to save product manufacturer");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setIsSaving(true);
    try {
      await deleteProductManufacturer(deleteTarget._id).unwrap();
      toast.success("Product Manufacturer deleted successfully");
      setDeleteTarget(null);
    } catch (err: any) {
      toast.error(err?.data?.message || "Failed to delete product manufacturer");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">Product Manufacturers</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Manage global manufacturer information registered for catalog products
          </p>
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900">
        <div className="border-b border-slate-100 p-4 dark:border-white/5">
          <ListEntitySearchPanel
            ...
            desktopPlaceholder="Search manufacturer name or description..."
            mobilePlaceholder="Search manufacturers..."
          />
        </div>
        ...
        <div className="relative overflow-x-auto">
          {isLoading ? (
            <div className="py-12 text-center text-slate-500">Loading Product Manufacturers...</div>
          ) : isError ? (
            <div className="py-12 text-center text-rose-500">Failed to load product manufacturers.</div>
          ) : paged.length === 0 ? (
            <div className="py-12 text-center text-slate-500">No product manufacturers found.</div>
          ) : (
                ...
                {paged.map((row) => (
                  <tr key={row._id} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.01]">
                    ...
                    <td className="px-4 py-3 text-right space-x-2">
                      <button
                        onClick={() => router.push(`${portalHome}/products?view=manufacturer-products&manufacturerId=${row._id}&manufacturerName=${encodeURIComponent(row.name)}`)}
          ...
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-white/10 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-white/5">
              <h2 className="font-semibold text-slate-955 dark:text-white">
                {editTarget ? "Edit Product Manufacturer" : "Create Product Manufacturer"}
              </h2>
              <button
                onClick={() => {
                  setCreateOpen(false);
            ...
            <form onSubmit={handleSave} className="p-4 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder="Add details about this manufacturer..."
                  rows={3}
                  className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-800 outline-none focus:border-blue-555 dark:border-white/10 dark:bg-slate-955 dark:text-slate-100 resize-none"
                />
              </div>
              ...
              <div className="flex flex-wrap gap-4">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor="man-is-active" className="text-xs text-slate-700 dark:text-slate-300 select-none cursor-pointer">
                    Is Active
                  </label>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="h-4 w-4 rounded border-slate-300 dark:border-slate-700 text-blue-600 focus:ring-blue-500 focus:ring-offset-0 cursor-pointer"
                  />
                  <label htmlFor="man-is-featured" className="text-xs text-slate-700 dark:text-slate-300 select-none cursor-pointer">
                    Is Featured
                  </label>
                </div>
              </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-white/10 p-6 space-y-4">
            <h3 className="font-bold text-slate-955 dark:text-white">Delete Product Manufacturer</h3>
            <p className="text-xs text-slate-505 dark:text-slate-400">
              Are you sure you want to delete Product Manufacturer <strong className="text-slate-955 dark:text-white">"{deleteTarget.name}"</strong>? Associated products will not be deleted but will lose manufacturer association.
            </p>
            <div className="flex justify-end gap-2 pt-2">
              <button
