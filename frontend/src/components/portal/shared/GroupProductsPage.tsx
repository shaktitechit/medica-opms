...
export type GroupProductsPageProps = {
  portalHome?: string;
};

export default function GroupProductsPage({
  portalHome = "/admin",
}: GroupProductsPageProps) {
  ...
  const { data: currentProducts = [], isLoading: isLoadingCurrent, refetch } = useGetProductGroupProductsQuery(groupId, {
    skip: !groupId,
  });
  ...
  const [associateProducts, { isLoading: isSaving }] = useAssociateProductGroupProductsMutation();
  ...
  const handleGoBack = () => {
    router.push(`${portalHome}/products?view=groups`);
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">
            Products under: <span className="text-blue-600 dark:text-blue-400">{groupName}</span>
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            View associated products or search the catalog to add items to this group
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
          ...
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900 overflow-hidden">
            ...
            <div className="divide-y divide-slate-100 dark:divide-white/5 max-h-[60vh] overflow-y-auto min-h-[300px]">
              {isLoadingCurrent ? (
                ...
                <div className="p-8 text-center text-xs text-slate-500">No products are currently associated with this group.</div>
              ) : (
        ...
        <div className="lg:col-span-5 space-y-3">
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900 overflow-hidden">
            <div className="border-b border-slate-100 p-4 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.01]">
              <h2 className="text-sm font-semibold text-slate-900 dark:text-white">Add Products to Group</h2>
            </div>
            ...
            <div className="p-4 border-b border-slate-100 dark:border-white/5">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-slate-200 bg-white pl-9 pr-4 py-2 text-xs text-slate-800 outline-none focus:border-blue-500 dark:border-white/10 dark:bg-slate-950 dark:text-slate-100"
                />
              </div>
            </div>
                  ...
                  return (
                    <div key={p._id} className="flex items-center justify-between p-3 hover:bg-slate-55/50 dark:hover:bg-white/[0.01]">
                      <div className="min-w-0 pr-4">
                        ...
                        <p className="text-[10px] text-slate-400 truncate">
                          {p.sku ? `SKU: ${p.sku} | ` : ""}
                          Group: {p.product_group?.name || p.product_group || "None"}
                        </p>
                      </div>
                      {isAssociated ? (
