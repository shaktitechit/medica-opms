...
/**
 * Park the page in the viewport for html2canvas. Capture the designed box
 * (offset size), not scrollHeight — overflowing content must be paginated in
 * the template, not sliced into extra blank PDF pages.
 */
async function capturePageElement(
  ...
  const width = Math.max(pageEl.offsetWidth, 1);
  const height = Math.max(pageEl.offsetHeight, 1);
...
export async function downloadOrderItemsPdf(
  ...
  if (pages.length > 0) {
    ...
    for (let i = 0; i < pages.length; i += 1) {
      ...
      const canvas = await capturePageElement(html2canvas, pageEl);
