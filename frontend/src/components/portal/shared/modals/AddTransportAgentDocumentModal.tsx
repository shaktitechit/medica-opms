...
export type AddTransportAgentDocumentModalProps = {
  open: boolean;
  transportAgentId: string;
  transportAgentLabel?: string;
  onClose: () => void;
  onUploaded?: () => void;
};

export function AddTransportAgentDocumentModal({
  open,
  transportAgentId,
  transportAgentLabel,
  onClose,
  onUploaded,
}: AddTransportAgentDocumentModalProps) {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        fd.append("entity_id", transportAgentId);
        fd.append("remarks", remarks.trim() || "Uploaded from transport agent documents tab");
    ...
    [createAttachment, file, onClose, onUploaded, remarks, transportAgentId],
  );
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="add-agent-doc-title"
      ...
      >
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4 px-5 py-4">
          <div className="space-y-1.5">
            <label htmlFor="agent-doc-file" className="text-xs font-medium text-slate-700 dark:text-slate-300">
          ...
          <div className="space-y-1.5">
            <label htmlFor="agent-doc-remarks" className="text-xs font-medium text-slate-700 dark:text-slate-300">
              Remarks
            </label>
            <textarea
              id="agent-doc-remarks"
              rows={2}
              value={remarks}
