...
export type AddDriverDocumentModalProps = {
  open: boolean;
  driverId: string;
  driverLabel?: string;
  onClose: () => void;
...
export function AddDriverDocumentModal({
  open,
  driverId,
  driverLabel,
  onClose,
  onUploaded,
}: AddDriverDocumentModalProps) {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        fd.append(
          "remarks",
          remarks.trim() || "Uploaded from driver documents tab",
        );
    ...
    [createAttachment, driverId, file, onClose, onUploaded, remarks],
  );
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="add-driver-doc-title"
      ...
      >
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4 px-5 py-4">
          <div className="space-y-1.5">
            <label htmlFor="drv-doc-file" className="text-xs font-medium text-slate-700 dark:text-slate-300">
          ...
          <div className="space-y-1.5">
            <label htmlFor="drv-doc-remarks" className="text-xs font-medium text-slate-700 dark:text-slate-300">
              Remarks
            </label>
            <textarea
              id="drv-doc-remarks"
