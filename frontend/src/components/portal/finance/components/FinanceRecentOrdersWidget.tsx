...
function renderWorkflowStatusBadge(category: FinanceOrderTabCategory) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-2xs font-bold uppercase tracking-wider ring-1 ring-inset ${bgClass}`}>
      {label}
    </span>
  );
}

export default function FinanceRecentOrdersWidget({
  orders,
  isOrdersFetching,
  isOrdersError,
  partyNameById,
  ...
  const filteredOrders = useMemo(() => {
    return orders
      ...
      .filter((o) => {
        // 1. Search Query filter (checks order ID or customer name)
        if (searchQuery.trim()) {
      ...
      .slice(0, 5); // Limit to top 5 recent matching results for dashboard usability
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-4 dark:border-white/5">
        <div>
          <h3 className="font-bold text-slate-900 dark:text-slate-100 font-sans">
            Finance Queue Explorer
          </h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 font-sans">
            Inspect, search, and filter the latest transactions in the billing approval queue
          </p>
        </div>
        <Link
          href="/finance/orders"
          className="text-xs font-semibold text-emerald-600 hover:underline dark:text-emerald-400 self-start sm:self-auto"
        >
          Manage All Orders
        </Link>
      </div>
      ...
      <div className="mt-4 flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-200 bg-white pl-9 pr-8 py-2 text-xs text-slate-900 outline-none transition focus:border-emerald-600 focus:ring-1 focus:ring-emerald-500/25 dark:border-white/10 dark:bg-slate-950 dark:text-slate-50 dark:focus:border-emerald-500"
          />
          {searchQuery && (
            <button
              type="button"
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs text-slate-900 outline-none transition focus:border-emerald-600 dark:border-white/10 dark:bg-slate-950 dark:text-slate-100 cursor-pointer"
          >
          ...
          {hasActiveFilters && (
            <button
              ...
              className="text-xs font-semibold text-emerald-600 hover:text-emerald-700 dark:text-emerald-450 pl-1 cursor-pointer whitespace-nowrap"
            >
              Reset
            </button>
          )}
  ...
  const deliveryStatusDim = deliveryBox?.status;
              ...
              return (
                <Link
                  key={id || ref}
                  href={`/finance/order/${id}`}
                  className="relative overflow-hidden rounded-xl border border-slate-200/80 bg-white p-4 transition-all duration-300 hover:shadow-md hover:border-emerald-500/20 dark:border-white/10 dark:bg-slate-900 flex flex-col gap-4 pl-5 cursor-pointer"
                >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/30 p-2.5 rounded-lg dark:bg-slate-955/5 border border-slate-100/50 dark:border-white/5">
                    <span className="text-slate-400 dark:text-slate-500 font-bold text-2xs uppercase tracking-wider font-sans">
