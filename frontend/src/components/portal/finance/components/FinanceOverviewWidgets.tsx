    ...
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6 font-sans">
      {FINANCE_ORDER_TABS.map((tab) => {
        ...
        return (
          <Link
            key={tab.id}
            href={`/finance/orders?tab=${tab.id}`}
            className="group relative overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-emerald-500/20 hover:shadow-md dark:border-white/10 dark:bg-slate-900 dark:hover:border-emerald-500/30"
          >
