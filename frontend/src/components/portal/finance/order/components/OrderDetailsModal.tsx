...
export default function OrderDetailsModal({
  ...
  return (
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/45 p-4 backdrop-blur-[2px]">
      <div className="w-full max-w-4xl rounded-xl border border-slate-200/90 bg-white p-5 shadow-xl dark:border-white/10 dark:bg-slate-900 transition-all max-h-[85vh] flex flex-col font-sans">
        ...
        <div className="mt-4 overflow-y-auto flex-1 pr-1">
          <dl className="grid gap-3 text-xs sm:grid-cols-2">
            <div>
              <dt className="text-xs font-medium text-slate-505 text-slate-500">Order No</dt>
              <dd className="mt-0.5 font-mono font-semibold text-slate-900 dark:text-slate-100">
                {String(detail.order_no ?? "—")}
              </dd>
            </div>
            <div>
              <dt className="text-xs font-medium text-slate-505 text-slate-500">Priority</dt>
            ...
            <div>
              <dt className="text-xs font-medium text-slate-505 text-slate-500">Order Date</dt>
              <dd className="mt-0.5 text-slate-900 dark:text-slate-100">
                {formatDate(detail.order_date)}
              </dd>
            </div>
            <div>
              <dt className="text-xs font-medium text-slate-505 text-slate-500">Expected Delivery</dt>
              <dd className="mt-0.5 text-slate-900 dark:text-slate-100">
                {formatDate(detail.expected_delivery_date)}
              </dd>
            </div>
            <div>
              <dt className="text-xs font-medium text-slate-505 text-slate-500">Created By</dt>
              <dd className="mt-0.5 font-semibold text-slate-900 dark:text-slate-100">
                {createdBy}
              </dd>
            </div>
            <div>
              <dt className="text-xs font-medium text-slate-505 text-slate-500">Created On</dt>
              <dd className="mt-0.5 text-slate-900 dark:text-slate-100">
                {formatDate(detail.createdAt)}
              </dd>
            </div>
            <div>
              <dt className="text-xs font-medium text-slate-505 text-slate-500">Last Modified</dt>
              <dd className="mt-0.5 text-slate-900 dark:text-slate-100">
                {formatDate(detail.updatedAt)}
              </dd>
            </div>
            <div>
              <dt className="text-xs font-medium text-slate-505 text-slate-500">Assigned Sales</dt>
              <dd className="mt-0.5 font-semibold text-slate-900 dark:text-slate-100">
                {resolveUser(detail.assigned_sales_user).name}
                {resolveUser(detail.assigned_sales_user).phone && (
                  <span className="ml-1 text-slate-550 dark:text-slate-400 font-normal">
                    ({resolveUser(detail.assigned_sales_user).phone})
                  </span>
                )}
              </dd>
            ...
            <div>
              <dt className="text-xs font-medium text-slate-505 text-slate-500">Assigned Admin</dt>
              <dd className="mt-0.5 font-semibold text-slate-900 dark:text-slate-100">
            ...
            <div>
              <dt className="text-xs font-medium text-slate-505 text-slate-500">Assigned Finance</dt>
              <dd className="mt-0.5 font-semibold text-slate-900 dark:text-slate-100">
            ...
            <div>
              <dt className="text-xs font-medium text-slate-505 text-slate-500">Assigned Dispatch</dt>
              <dd className="mt-0.5 font-semibold text-slate-900 dark:text-slate-100">
            ...
            <div className="sm:col-span-2">
              <dt className="text-xs font-medium text-slate-505 text-slate-500">Remarks</dt>
