...
type AdminAmendSalesApprovalModalProps = {
  open: boolean;
  onClose: () => void;
...
function adminOverrideLineStatus(approvedQty: number): LineStatus {
...
export function AdminAmendSalesApprovalModal({
  open,
  onClose,
  approval,
  orderId,
  ...
  refetchOrder,
  onAmended,
}: AdminAmendSalesApprovalModalProps) {
  ...
  const [amendAdminApproval, { isLoading: isAmending }] =
    useAmendOrderApprovalMutation();
      ...
      setFormLines((prev) =>
        prev.map((line) => {
          ...
          if (patch.approved_quantity !== undefined) {
            ...
            next.approval_status = adminOverrideLineStatus(qty);
          }
  ...
  const submitAmendment = useCallback(async () => {
    ...
    try {
      ...
      toast.success("Admin approval amended successfully.");
      onClose();
      onAmended?.();
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-6 py-4 space-y-4">
              ...
              {addableOrderLines.length > 0 ? (
                <div className="space-y-2">
                  <h4 className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                    Add Existing Order Lines Not In Admin Approval:
                  </h4>
              ...
              <div className="grid gap-4 sm:grid-cols-1">
                <div className="space-y-1">
                  <label htmlFor="amendmentNotes" className={labelClass}>Amendment notes</label>
                  <textarea
                    id="amendmentNotes"
                    rows={3}
                    placeholder="Reason for admin amendment…"
...
export default AdminAmendSalesApprovalModal;
