...
import WorkPlansPdfTemplate from "./WorkPlansPdfTemplate";
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      await downloadOrderItemsPdf(
        ...
        `admin_work_plans_${dateStamp}.pdf`,
          ...
          <div className="flex shrink-0 flex-wrap items-start justify-between gap-3 border-b border-slate-100 px-4 py-3 dark:border-white/10 sm:px-5">
            <div>
              ...
              <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                Filter by period and sales executive, then download as PDF (includes all visits)
              </p>
            </div>
