...
import { buildSettlementPreviewLines } from "@/components/portal/shared/returnSettlement";
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200/60 bg-slate-50/80 px-3 py-2 dark:border-white/5 dark:bg-slate-950/40">
        <div>
          ...
          <p className="text-[10px] text-slate-500 dark:text-slate-400">
            Dispatched vs returned (from dispatch batches)
          </p>
        </div>
        <span
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-white/5">
          {section.rows.map((row) => (
            <tr key={row.orderItemId} className="bg-white dark:bg-slate-900">
              ...
              <td className="px-3 py-2 text-center tabular-nums font-semibold text-rose-600 dark:text-rose-400">
                {row.atWarehouseQty}
              </td>
...
export function CloseAccountOrderModal({
  ...
  const releaseSections = useMemo(
    () => buildSettleCloseReleaseSections(approvals, orderItems, dispatches),
    [approvals, orderItems, dispatches],
  );
  ...
  const releasesReopenedByReturns = useMemo(() => {
    return releaseSections.filter((section) => {
      if (!section.needsResolve) return false;
      const approval = approvals.find(
        (row) => String(row._id ?? row.id ?? "") === section.approvalId,
      );
      return approval ? isDispatchReleaseResolved(approval) : false;
    }).length;
  }, [releaseSections, approvals]);

  const previewLines = useMemo(
    () => buildSettlementPreviewLines(orderItems, dispatches),
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        releasesToResolve > 0
          ? "Order settled and closed — releases resolved to net dispatch quantities."
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/45 p-4 backdrop-blur-[1px]">
      <div className="w-full max-w-3xl rounded-xl border border-slate-200/90 bg-white shadow-xl dark:border-white/10 dark:bg-slate-900 max-h-[92vh] flex flex-col">
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-white/5">
          <div>
            ...
            <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
              Returns are taken from dispatch batch line items. Net quantities settle the order, then
              it closes as{" "}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5 font-sans">
          {releasesToResolve > 0 ? (
            <div className="flex gap-3 rounded-lg border border-amber-200/80 bg-amber-50/70 p-3 text-xs text-amber-900 dark:border-amber-900/30 dark:bg-amber-950/20 dark:text-amber-200">
              ...
              <p>
                {releasesReopenedByReturns > 0 ? (
                  <>
                    {releasesReopenedByReturns} release batch
                    {releasesReopenedByReturns === 1 ? "" : "es"} reopened due to new dispatch
                    returns and will be re-settled on close.
                    {releasesToResolve > releasesReopenedByReturns ? (
                      <>
                        {" "}
                        {releasesToResolve - releasesReopenedByReturns} additional release
                        {releasesToResolve - releasesReopenedByReturns === 1 ? "" : "s"} also
                        require settlement.
                      </>
                    ) : null}
                  </>
                  ...
                  <>
                    ...
                    resolved to net dispatched quantities (after dispatch returns) before close.
                  </>
                )}
              </p>
            </div>
          ...
          <div className="rounded-xl border border-slate-200/80 bg-slate-50/60 p-4 dark:border-white/10 dark:bg-slate-950/30">
            ...
            <dl className="space-y-1.5 text-sm">
              ...
              <div className="flex justify-between text-xs text-slate-500 pt-1">
                <dt>Total returned (from dispatch)</dt>
        ...
        <div className="flex justify-end gap-3 border-t border-slate-100 px-5 py-4 dark:border-white/5">
          ...
          >
            {isLoading ? "Settling & closing…" : "Confirm Settle & Close"}
          </button>
        </div>
      </div>
    </div>
