...
export function SettleRestOrderModal({
  ...
  const [settleRelease, { isLoading: isResolving }] =
    useResolvePartialDispatchReleaseMutation();
  const [patchOrder, { isLoading: isPatchingOrder }] = usePatchOrderMutation();
  const isLoading = isResolving || isPatchingOrder;
  ...
  /** Order lines amended to match settled quantities for this release. */
  const settledOrderItems = useMemo(() => {
    const byLine = new Map(
      previewRows.map((row) => [row.orderItemId, row] as const),
    );
    ...
    for (const line of orderItems) {
      const lineId = idFromRef(line._id ?? line.id);
      const row = byLine.get(lineId);
      if (!row) {
        next.push({
          ...line,
          product: idFromRef(line.product) || line.product,
          _id: lineId || line._id,
        });
        continue;
      }
      if (row.settledQty <= 0) continue;
      next.push({
        ...line,
        product: idFromRef(line.product) || line.product,
        _id: lineId || line._id,
        ordered_quantity: row.settledQty,
        quantity: row.settledQty,
        approved_quantity: row.settledQty,
      });
    }

    return next;
  }, [orderItems, previewRows]);
  ...
  const handleSettle = async () => {
    ...
    try {
      // 1) Amend the finance/account approval batch to net settled quantities.
      await settleRelease({
        id: approvalId,
        body: {
          amendment_notes:
            notes.trim() ||
            "Settled remaining release clearance to match dispatched quantities and warehouse returns",
          approval_items: settledApprovalItems,
        },
      }).unwrap();

      // 2) Amend the order line quantities to the same settled amounts.
      try {
        await patchOrder({
          id: orderId,
          patch: { order_items: settledOrderItems },
        }).unwrap();
      } catch {
        // Resolve-dispatch already amends the order; keep settle successful if this reinforce patch fails.
      }

      toast.success(
        "Rest order settled — approval batch and order quantities both updated.",
      );
      setNotes("");
      onClose();
      onSettled?.();
          ...
          <div className="flex items-start justify-between border-b border-slate-100 px-6 py-4 dark:border-white/5">
            <div>
              ...
              <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                Release {batchLabel} — amends this approval batch and the order
                to net settled quantities
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-6 py-4 space-y-4">
            <div className="flex gap-3 rounded-lg border border-amber-200/80 bg-amber-50/70 p-3 text-xs text-amber-900 dark:border-amber-900/30 dark:bg-amber-950/20 dark:text-amber-200">
              ...
              <p>
                ...
                {totals.settledReturnsQty > 0
                  ...
                  : ""}{" "}
                will be removed. Net settled total:{" "}
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                placeholder="Reason for settling remaining clearance…"
              />
            </div>
          </div>
