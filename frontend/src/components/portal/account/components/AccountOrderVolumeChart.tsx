...
import {
  ...
  categorizeOrderForAccountChart,
  createEmptyAccountChartBreakdown,
  orderLineQuantity,
  type AccountChartBreakdown,
  type AccountOrderCategoryOptions,
  type AccountOrderTabCategory,
} from "../accountOrderUtils";

interface AccountOrderVolumeChartProps {
  orders: unknown[];
...
type ChartBucket = {
  ...
  totalQty: number;
  totalVolume: number;
  breakdown: AccountChartBreakdown;
};
...
function buildMonthlyBuckets(
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    months.push({
      ...
      totalQty: 0,
      totalVolume: 0,
      breakdown: createEmptyAccountChartBreakdown(),
    });
  }

  for (const order of orders) {
    const cat = categorizeOrderForAccountChart(order, categoryOptions);
    if (!cat) continue;
...
export default function AccountOrderVolumeChart({
  orders,
  isOrdersFetching,
  categoryOptions,
}: AccountOrderVolumeChartProps) {
  ...
  const totalOrdersCount = useMemo(
    () =>
      orders.filter((o) => categorizeOrderForAccountChart(o, categoryOptions) !== null).length,
  ...
  const statusKeys = ACCOUNT_ORDER_TABS.map((tab) => tab.id);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-4 dark:border-white/5">
        <div>
          <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100 font-sans">
            Account Analytics & Value Volume
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 font-sans">
            Track assigned order volumes, line quantities, and revenue values in real time
          </p>
        </div>
              ...
              return (
                ...
                >
                  {statusKeys.map((statusKey) => {
                    ...
                    const colorInfo = ACCOUNT_STATUS_COLORS[statusKey];
              ...
              (() => {
                ...
                const activeBreakdowns = statusKeys
                  .map((key) => {
                    ...
                    return {
                      key,
                      label: ACCOUNT_STATUS_COLORS[key].label,
                      val: segmentVal,
                    };
                  })
                  .filter((ab) => ab.val > 0);
                ...
                return (
                  <g className="pointer-events-none transition-all duration-200">
                    ...
                    {activeBreakdowns.map((ab, idx) => {
                      ...
                      const colorInfo = ACCOUNT_STATUS_COLORS[ab.key];
                      const displayVal =
