...
interface AccountRecentOrdersWidgetProps {
  orders: unknown[];
...
export default function AccountRecentOrdersWidget({
  orders,
  isOrdersFetching,
  isOrdersError,
  partyNameById,
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-4 dark:border-white/5">
        <div>
          <h3 className="font-bold text-slate-900 dark:text-slate-100 font-sans">
            Account Queue Explorer
          </h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 font-sans">
            Inspect, search, and filter your latest assigned orders across the fulfillment pipeline
          </p>
        </div>
        <Link
          href="/account/orders"
          className="text-xs font-semibold text-blue-600 hover:underline dark:text-blue-400 self-start sm:self-auto"
        >
          Manage All Orders
        </Link>
      </div>
      ...
      <div className="mt-4 flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-200 bg-white pl-9 pr-8 py-2 text-xs text-slate-900 outline-none transition focus:border-blue-600 focus:ring-1 focus:ring-blue-500/25 dark:border-white/10 dark:bg-slate-950 dark:text-slate-50 dark:focus:border-blue-500"
          />
          {searchQuery && (
            <button
              type="button"
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs text-slate-900 outline-none transition focus:border-blue-600 dark:border-white/10 dark:bg-slate-950 dark:text-slate-100 cursor-pointer"
          >
          ...
          {hasActiveFilters && (
            <button
              ...
              className="text-xs font-semibold text-blue-600 hover:text-blue-700 dark:text-blue-450 pl-1 cursor-pointer whitespace-nowrap"
            >
              Reset
            </button>
          )}
          ...
          <div className="py-10 text-center border border-dashed border-slate-200 dark:border-white/5 rounded-xl">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
              {hasActiveFilters
                ...
                : "No orders currently assigned to you in the pipeline."}
            </p>
            {hasActiveFilters && (
              <button
                type="button"
