...
export default function SalesMonthlyPerformanceChart({
  orders,
  isOrdersFetching,
      ...
      <div className="mt-6 relative z-20 h-[280px] w-full flex items-center justify-center">
        {isOrdersFetching ? (
          ...
          <svg viewBox="0 0 640 230" className="w-full h-full select-none overflow-visible">
              ...
              return (
                <g key={label}>
                  {activeYears.map((year, yearIdx) => {
                    const qty = monthlyByYear.get(year)?.[monthIdx] ?? 0;
                    const barHeight = (qty / maxVal) * 170;
            ...
            {hoveredCell &&
              (() => {
                const qty = monthlyByYear.get(hoveredCell.year)?.[hoveredCell.month] ?? 0;
                const slotWidth = 568 / 12;
