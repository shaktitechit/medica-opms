...
interface SalesFeaturedProductGroupFeaturedPartyTableProps {
  orders: any[];
...
/**
 * Featured groups × featured parties (quantity only).
 * Orders passed in should already be scoped to the logged-in sales user.
 */
export default function SalesFeaturedProductGroupFeaturedPartyTable({
  orders,
  isOrdersFetching,
}: SalesFeaturedProductGroupFeaturedPartyTableProps) {
  ...
  const handleDownload = () => {
    ...
    downloadCsvFile(
      reportFilename("sales_product_group_party", selectedYears, selectedMonths),
      headers,
      rows,
      [
  ...
  return (
    <FeaturedMatrixTableFrame
      ...
      subtitle={
        qtyBasis === "net"
          ? "Net quantity by product group (expandable to products) across featured parties (your orders)"
          : "Approved quantity by product group (expandable to products) across featured parties (your orders)"
      }
