...
export function ExpenseListSection({
  ...
  const canAddExpense = canManage && canAddExpenseForPlanDate(planDate);
  const addWindowHint = expenseAddWindowHint(planDate);
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-white/10 dark:bg-slate-900">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-3 dark:border-white/10">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {canManage ? (
            <button
              type="button"
              disabled={busy || !canAddExpense}
              title={canAddExpense ? "Add expense" : addWindowHint}
              onClick={() => {
      ...
      {canManage && !canAddExpense ? (
        <div className="border-b border-amber-100 bg-amber-50 px-4 py-2 text-[11px] text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-200">
          {addWindowHint}
        </div>
      ) : null}
