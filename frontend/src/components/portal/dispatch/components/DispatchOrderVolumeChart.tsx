...
import {
  ...
  categorizeOrderForDispatchChart,
  createEmptyDispatchChartBreakdown,
  orderLineQuantity,
  type DispatchChartBreakdown,
...
type ChartBucket = {
  ...
  totalQty: number;
  totalVolume: number;
  breakdown: DispatchChartBreakdown;
};
...
function accumulateOrderIntoBucket(
  bucket: ChartBucket,
  order: unknown,
  cat: DispatchOrderTabCategory,
) {
  bucket.ordersCount++;
...
function buildMonthlyBuckets(
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    months.push({
      ...
      totalQty: 0,
      totalVolume: 0,
      breakdown: createEmptyDispatchChartBreakdown(),
    });
  }

  for (const order of orders) {
    const cat = categorizeOrderForDispatchChart(order, categoryOptions);
    if (!cat) continue;
...
export default function DispatchOrderVolumeChart({
  orders,
  isOrdersFetching,
  categoryOptions,
}: DispatchOrderVolumeChartProps) {
  ...
  const totalOrdersCount = useMemo(
    () =>
      orders.filter((o) => categorizeOrderForDispatchChart(o, categoryOptions) !== null).length,
    [orders, categoryOptions],
  );

  const monthlyData = useMemo(
    () => buildMonthlyBuckets(orders, categoryOptions),
    [orders, categoryOptions],
  );
  const dailyData = useMemo(
    () => buildDailyBuckets(orders, categoryOptions),
    [orders, categoryOptions],
  );

  const activeData = timeframe === "monthly" ? monthlyData : dailyData;
  const statusKeys = DISPATCH_CHART_TABS.map((tab) => tab.id);
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-900 font-sans relative z-10">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-4 dark:border-white/5">
        <div>
          <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100 font-sans">
            Dispatch Analytics & Value Volume
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 font-sans">
            Track transport volumes, line quantities, and shipment values in real time
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex rounded-lg bg-slate-100 p-1 dark:bg-slate-800">
            <button
              ...
              className={`rounded-md px-3 py-1.5 text-xs font-semibold transition cursor-pointer ${metricButtonClass(showMetric === "orders")}`}
            >
              Orders
            </button>
            <button
              ...
              className={`rounded-md px-3 py-1.5 text-xs font-semibold transition cursor-pointer ${metricButtonClass(showMetric === "quantities")}`}
            >
              Quantities
            </button>
            <button
              ...
              className={`rounded-md px-3 py-1.5 text-xs font-semibold transition cursor-pointer ${metricButtonClass(showMetric === "volume")}`}
            >
              Order Value (₹)
            </button>
          </div>
          ...
          <div className="flex rounded-lg bg-slate-100 p-1 dark:bg-slate-800">
            <button
              ...
              className={`rounded-md px-3 py-1.5 text-xs font-semibold transition cursor-pointer ${metricButtonClass(timeframe === "monthly")}`}
            >
              Monthly
            </button>
            <button
              ...
              className={`rounded-md px-3 py-1.5 text-xs font-semibold transition cursor-pointer ${metricButtonClass(timeframe === "daily")}`}
            >
              Daily (10d)
            </button>
          </div>
      ...
      <div className="mt-3.5 flex flex-wrap items-center gap-x-4 gap-y-2 text-2xs font-semibold text-slate-500 dark:text-slate-400 border-b border-slate-100/50 pb-3 dark:border-white/5">
        {statusKeys.map((key) => {
          const colorInfo = DISPATCH_STATUS_COLORS[key];
          return (
              ...
              return (
                ...
                >
                  {statusKeys.map((statusKey) => {
                    ...
                    const colorInfo = DISPATCH_STATUS_COLORS[statusKey];
              ...
              (() => {
                ...
                const activeBreakdowns = statusKeys
                  .map((key) => {
                    ...
                    return {
                      key,
                      label: DISPATCH_STATUS_COLORS[key].label,
                      val: segmentVal,
                    };
                  })
                  .filter((ab) => ab.val > 0);
                ...
                return (
                  <g className="pointer-events-none transition-all duration-200">
                    ...
                    {activeBreakdowns.map((ab, idx) => {
                      ...
                      const colorInfo = DISPATCH_STATUS_COLORS[ab.key];
                      const displayVal =
