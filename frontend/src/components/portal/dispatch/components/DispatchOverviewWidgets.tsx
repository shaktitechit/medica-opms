        ...
        <div className="grid grid-cols-3 lg:grid-cols-11 gap-4 w-full">
          {DISPATCH_ORDER_TABS.map((tab) => {
            ...
            return (
              <Link
                key={tab.id}
                href={`/dispatch/orders?tab=${tab.id}`}
                className="group relative overflow-hidden rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-amber-500/20 hover:shadow-md dark:border-white/10 dark:bg-slate-900 dark:hover:border-amber-500/30 flex flex-col justify-between"
              >
