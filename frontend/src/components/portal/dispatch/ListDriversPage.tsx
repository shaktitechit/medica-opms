...
export default function ListDriversPage() {
  ...
  }, [drivers, searchQuery, vehicleMap]);
      ...
      <div className="rounded-xl border border-slate-200 bg-white dark:border-white/10 dark:bg-slate-900 shadow-sm overflow-hidden">
        {isFetching && (
          <div className="flex flex-col items-center justify-center py-16 space-y-2">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Loading drivers...</p>
          </div>
        )}
        ...
        {!isFetching && !isError && filteredDrivers.length === 0 && (
          <div className="text-center py-16 px-4">
            ...
            <p className="mt-1.5 text-xs text-slate-500 max-w-xs mx-auto">
              {drivers.length === 0
                ? "No fleet drivers are registered in the system yet."
                  ...
                  {paginatedDrivers.map((d) => {
                    ...
                    if (d.assigned_vehicle) {
                      if (typeof d.assigned_vehicle === "object") {
                        ...
                        vehicleLabel =
                          vehicleMap.get(assignedVehicleId) || "—";
                      }
                    }
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3.5 font-mono text-[11px] font-semibold text-slate-700 dark:text-slate-300">
                          {code}
                        </td>
                        ...
                        <td className="px-4 py-3.5 tabular-nums text-slate-650 dark:text-slate-350 font-medium">
                          {phone}
                        </td>
                        ...
                        <td className="px-4 py-3.5 font-bold uppercase tracking-tight font-mono">
                        ...
                        <td className="px-4 py-3.5">
                          {statusStr === "available" ? (
                            <span className="inline-flex items-center rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-700 ring-1 ring-inset ring-emerald-700/10 dark:bg-emerald-950/30 dark:text-emerald-455/90 dark:ring-emerald-500/25">
                              Available
                            </span>
                          ) : statusStr === "assigned" ? (
                            <span className="inline-flex items-center rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-blue-700 ring-1 ring-inset ring-blue-700/10 dark:bg-blue-950/30 dark:text-blue-400 dark:ring-blue-500/25">
                              Assigned
                            </span>
                          ) : statusStr === "on_trip" ? (
                            <span className="inline-flex items-center rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-amber-700 ring-1 ring-inset ring-amber-700/10 dark:bg-amber-950/30 dark:text-amber-400 dark:ring-amber-500/20">
                              On trip
                            </span>
                          ) : statusStr === "leave" ? (
                            <span className="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-700 ring-1 ring-inset ring-slate-600/10 dark:bg-white/5 dark:text-slate-400 dark:ring-white/10">
                              Leave
                            </span>
                          ) : statusStr === "inactive" ? (
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between px-4 py-3 border-t border-slate-200/60 dark:border-white/5 bg-slate-50/50 dark:bg-slate-950/20 text-slate-600 dark:text-slate-400">
              <div className="flex flex-wrap items-center gap-3">
                <span className="text-xs">
                  ...
                  <span className="font-semibold text-slate-800 dark:text-slate-200">{filteredDrivers.length}</span> entries
                </span>
